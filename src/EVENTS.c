...
int EV_HandleEvents() {
    ...
    while (SDL_PollEvent(&event)) {
        switch (event.type) {
            ...
            case SDL_WINDOWEVENT: handleWin(&event); break;

            case SDL_KEYDOWN: handleKeyboard(&event); break;

            case SDL_JOYBUTTONUP:
            case SDL_JOYBUTTONDOWN:
                if (handleJoyButtons(&event) == -1) status = 1;
                break;
            case SDL_JOYAXISMOTION: handleJoyAxis(&event); break;
            case SDL_JOYHATMOTION: handleJoyHat(&event); break;
...
static void handleKeyboard(SDL_Event *event) {
    ...
    if (cmd) { write(term.childfd, cmd, SDL_strlen(cmd)); }
}
...
int handleJoyAxis(SDL_Event *ev) {
    ...
    switch (ev->jaxis.axis) {
        ...
        case 2:  // Left Trigger
            ...
            if (ev->jaxis.value == 32767) KEYB_SimulateKey(SDLK_F3, STATE_TYPED);
            break;
        case 3:
        ...
        case 5:  // Right Trigger
            if (ev->jaxis.value == 32767) KEYB_SimulateKey(SDLK_F4, STATE_TYPED);
            break;
    }
    return 0;
}
